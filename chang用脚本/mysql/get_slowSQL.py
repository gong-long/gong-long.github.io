...
if __name__ == "__main__":
    '''判断sql文件是否存在，
       若存在则将内容清空
    '''
    if os.path.exists('/home/qmzb/sql.txt'):
        os.system('> /home/qmzb/sql.txt')
    for db in dblist:
        try:
            with open("/home/qmzb/sql.txt",'a') as f:
                for i in exc_sql(db)['Items']['SQLSlowRecord']:
                    b = i['DBName']+":"+i['SQLText'].strip().replace('\n','')
                    matchObj = re.match( r'(.*):(.*)40001 SQL_NO_CACHE.*',b, re.M|re.I)
